...
if [ "$http_code" -ge 400 ]; then
  echo "WARNING: POST /api/leads/${lead_id}/run returned ${http_code}: ${body}" >&2
fi

extraction=$(echo "$body" | jq '.data.extraction')
if [ "$extraction" = "null" ] || [ -z "$extraction" ]; then
  echo "ERROR: extraction key missing from run response" >&2
  echo "$body" >&2
  exit 1
fi
...
if [ -n "$no_perm_id" ]; then
  no_perm_run=$(curl -s -X POST "${BASE_URL}/api/leads/${no_perm_id}/run" \
    -H 'Content-Type: application/json' \
    -d '{"modules":["extraction"]}')
  no_perm_status=$(echo "$no_perm_run" | jq -r '.data.extraction.status // empty')
  no_perm_reason=$(echo "$no_perm_run" | jq -r '.data.extraction.reason // empty')
